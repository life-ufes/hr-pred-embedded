...
{
    // If hardware acceleration is enabled, use optimized DSP functions. Otherwise, use the ANSI C version.
    ...
    dsps_sqrt_f32_ansi(out, out, len);            // out = sqrt(out)
#else
    dsps_mul_f32_ansi(x, x, out, len, 1, 1, 1);   // out = x * x
    dsps_mul_f32_ansi(y, y, y, len, 1, 1, 1);     // y = y * y
    dsps_add_f32_ansi(out, y, out, len, 1, 1, 1); // out = out + y
    dsps_mul_f32_ansi(z, z, z, len, 1, 1, 1);     // z = z * z
    dsps_add_f32_ansi(out, z, out, len, 1, 1, 1); // out = out + z
    dsps_sqrt_f32_ansi(out, out, len);            // out = sqrt(out)

    //// Same as above but without using DSP functions, just for reference:
    // for (int i = 0; i < len; i++)
    // {
