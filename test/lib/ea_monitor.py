...
class EAModelMonitor:
    ...
    def _on_serial_rx(self, data: bytes):
        ...
        while len(self.raw_buffer) >= 7:
            ...
            logger.debug("[Serial] Received length: %d bytes with header at index: %d", len(self.raw_buffer), header_index)
            ...
            if calc_checksum == received_checksum:
                ...
                logger.info("[RX] Erro de Checksum no pacote tipo %s", pkt_type)
    ...
    def _handle_valid_packet(self, pkt_type: int, payload: bytes):        
        if pkt_type == 0x01: # DATA (telemetry)
            try:
                ...
                logger.debug("[RX - Data] HR: %.2f | HR_REG: %.2f | HR_GT: %.2f\nAL: %.2f | AL_RAW: %.2f", hr, hr_reg, hr_gt, al, al_raw)
                self.metrics.add_sample(sample)
                
            except struct.error:
                logger.info("[RX] Erro ao descompactar floats de telemetria")

        elif pkt_type == 0x02: # LOG (Strings do ESP_LOG)
            try:
                message = payload.decode('utf-8', errors='ignore').strip()
                logger.info("[ESP32 LOG] %s", message)
            except Exception as e:
                logger.info("[RX] Erro ao decodificar string de log: %s", e)


    # ----------------------------------
    def _on_data_ready(self, data: list):
        if self.debug:
            logger.debug("[TX - Ground Truth] %s BPM", data[-1])
            
        raw_payload = struct.pack(f"<{len(data)}f", *data)
